<div class="col-md-12 green" >
	...
	<ul class="nav navbar-nav navbar-right moredeep">
		...
		<a href="#" class="green-none white" data-toggle="dropdown">
				<img src="/image/${(user.img_path)!'/timg.jpg'}" class="user-image" /> <span>${user.user_name}</span>
		</a> <!--设置点击按钮弹出用户面板层-->
			<ul class="dropdown-menu" style="padding:0;">
				<li class="user-header green"><img src="/image/${(user.img_path)!'/timg.jpg'}"
					class="img-circle" style="width: 100px;height:100px;" />
					<p class="white" style="">
						<span>${user.dept.dept_name} </span> / <span> ${user.role.role_name}</span><br> <small>${user.position.name}</small>
					</p></li>
